...
export const Header: React.FC = () => {
  const { 
    currentUser, users, setCurrentUserById, activePage, navigateTo, 
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (roleRef.current && !roleRef.current.contains(e.target as Node)) setIsRoleSwitcherOpen(false);
    };
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1 font-semibold text-emerald-400">
              ...
              EasyMart Escrow Protection
            </span>
            <span className="hidden sm:inline text-neutral-500">•</span>
            <span className="hidden sm:inline text-neutral-300">Funds released only after you verify the parcel delivery.</span>
          </div>

          <div className="flex items-center gap-4 text-[11px]">
            <span className="text-amber-300 font-medium hidden md:inline">
              ⚡ Everyday Essentials Only (Strictly No Heavy Machinery)
            </span>
            <div className="relative" ref={roleRef}>
              <button 
                id="header-role-switcher"
                onClick={() => setIsRoleSwitcherOpen(!isRoleSwitcherOpen)}
                className="flex items-center gap-1 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 px-2 py-0.5 rounded text-xs transition border border-neutral-700"
              >
                <span>Demo Profile: <strong className="text-emerald-400 capitalize">{currentUser.role}</strong> ({currentUser.name.split(' ')[0]})</span>
                <ChevronDown className="w-3 h-3 text-neutral-400" />
              </button>

              {isRoleSwitcherOpen && (
                <div className="absolute right-0 mt-1.5 w-64 bg-white text-neutral-800 rounded-lg shadow-xl border border-neutral-200 py-1.5 z-50 text-xs">
                  <div className="px-3 py-1 text-[11px] font-semibold text-neutral-400 uppercase tracking-wider border-b border-neutral-100">
                    Switch Test Persona
                  </div>
                  {users.map(u => (
                    <button
              ...
              {isUserMenuOpen && (
                <div className="absolute right-0 mt-2 w-60 bg-white rounded-2xl shadow-2xl border border-neutral-200 py-2 z-50 text-xs">
                  ...
                  <div className="pt-1 border-t border-neutral-100">
                    <button
                      onClick={() => { navigateTo('login'); setIsUserMenuOpen(false); }}
